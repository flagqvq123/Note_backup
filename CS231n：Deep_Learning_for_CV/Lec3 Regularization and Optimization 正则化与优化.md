...
- example：正则化如何与这个优化器作用？(比如L2正则化)
	- A：不同情况下有不同的作用方式！
	- 默认情况下，Adam将正则化损失置于梯度计算前，这样在前进过程中的动量方向和位移都会受到正则化项影响。
	- AdamW只将正则化项放在数据的直接计算部分（位移）。
	- 正则化的方法有很多。AdamW表现会更好一点。

##### Learning rate 学习率选择
- 太高的步长导致穿过低谷或者持续震荡，太低的步长导致收敛缓慢
- 更好的简单方法：在学习中改变学习率/步长
	- 经过一定数量次数的迭代减少学习率
	- 使用一个递减函数让学习率随迭代次数减少
		- 余弦、线性、反平方根......
		- 选择效果最好的就行
	- 真正流行的策略：先线性预热上升，再减少。
	- 如果你的batch数量线性提升，也应该等比例提升学习率

- 二阶优化：对于每一步，使用二次项来优化而不是线性的偏导。具体来说，就是将该点的二阶偏导数也纳入考虑。
	- 这样的方式可能会带来更复杂的计算，同时参数会变得很巨大，导致内存溢出......
	- 并且优化效果可能也不会更好（？）
